...
> Read this first when returning to the project. Last updated: 2026-07-22 (M9 started: added a persisted **outcome pill** to the shared `AssessmentList` component, so the global `/assessments` list and each project's instance list show Suitable/Not suitable next to the status pill for Complete assessments — see the M9 row below. Earlier the same day: Letter got a genuine **Word (.docx) export** alongside the PDF — `docx` library, a shared `letterData.ts` data layer so the two exports can never disagree, and an HTML→docx run converter that preserves rich-text formatting. Also added a **Seed letter demo** seeder + `demo-guide.md` §11 for showcasing the M8b.2 Grouped-subsections block end to end. M8b (custom letter builder + grouped subsections) remains fully shipped — see below.)
...
| **M9** NFRs, QA, deployment | in progress | The remaining real milestone. **Done:** Persisted outcome pill on the assessment list — `AssessmentList.tsx` (shared by the global `/assessments` page and the project-detail page) now shows a green "Suitable" / red "Not suitable" pill next to the status pill for `Complete` instances, reading `dnx_outcome` directly (mirrors the pill already on the Dashboard's recent-assessments table). **TODO:** (1) **Role-gated actions** — gate Submit / Reopen / Delete / Approve / Reject / Mark resolved / AI auto-fill by Entra/Dataverse role (Assessor / Reviewer / Admin); everything is visible to everyone today — biggest correctness gap before real users. (2) **a11y** — axe-core scan + keyboard nav across every field type. (3) **Perf** — 200-question template render ≤ 3 s (PRD §8), virtualize long sections with `@tanstack/react-virtual` if needed. Smaller polish: code-split with `React.lazy()` (bundle ~1.68 MB / 474 kB gzipped); throttle audit snapshots (skip when < ~30 s since last Autosave); multi-value visibility rules; mobile responsiveness audit; clean up the two baseline `react-hooks/set-state-in-effect` lint errors (`AssessmentPage` + `LevelDialog`). |
