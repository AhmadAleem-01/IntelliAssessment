...
> Read this first when returning to the project. Last updated: 2026-06-07 (M6b refined: AI evidence bindings now authored in a dedicated **AI conditioning** tab on the template editor; assessment-time auto-fill offers all bound questions — answered + hidden — with a per-question checklist so the assessor controls exactly which ones hit the AI; mapping persists in `dnx_evidence_mapping`).
