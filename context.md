...
> Read this first when returning to the project. Last updated: 2026-05-31 (M7c persistence + hero chip + Approve guards + author-able assessment outcome shipped).
...
| **M7d** Grouped scoring | not started | `dnx_scoring_groups` table — multiple criteria bound into one scoring group with per-group weights. Currently falls back to Boolean semantics in the aggregator. |
