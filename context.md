...
> Read this first when returning to the project. Last updated: 2026-06-09 (M8b **custom letter builder** shipped: a drag-drop block designer on a new **Letter** tab — heading / rich-text / details-grid / outcome / reviewer-notes / responses / signature / spacer blocks, inline answer-reference **chips** you can colour, a `/` slash menu + picker to insert them, and a formatting toolbar (B/I/U, size, colour, align, lists). Layout persists as sanitized HTML in `dnx_letter_template_json`. Reason-grouped qualifications still deferred. Also fixed: clearing a date answer sent `''` to `Edm.Date` → now sends null.)
...
| **M9** NFRs, QA, deployment | not started | The remaining real milestone. **TODO:** (1) **Role-gated actions** — gate Submit / Reopen / Delete / Approve / Reject / Mark resolved / AI auto-fill by Entra/Dataverse role (Assessor / Reviewer / Admin); everything is visible to everyone today — biggest correctness gap before real users. (2) **Persisted outcome pill on the global `/assessments` list** — it shows status pills but not the computed/persisted outcome. (3) **a11y** — axe-core scan + keyboard nav across every field type. (4) **Perf** — 200-question template render ≤ 3 s (PRD §8), virtualize long sections with `@tanstack/react-virtual` if needed. Smaller polish: code-split with `React.lazy()` (bundle ~1.68 MB / 474 kB gzipped); throttle audit snapshots (skip when < ~30 s since last Autosave); multi-value visibility rules; mobile responsiveness audit; clean up the two baseline `react-hooks/set-state-in-effect` lint errors (`AssessmentPage` + `LevelDialog`). |
...
What's left is folded into the **Where we are right now** milestone table near the top — see the `TODO:` notes on the **M6b** (AI follow-ups), **M8b** (letter builder, deferred), and **M9** (hardening + polish) rows.
