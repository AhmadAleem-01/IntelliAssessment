...
- **`design.md`** at the repo root — the **single canonical UI design system** ("Calm Efficiency" v1.0). Navy/blue identity, violet reserved for AI only, `--ds-*` tokens in `index.css`, honesty rule (never fabricate metrics). All new/migrated pages follow it. (An earlier flat-purple spec + a separate `newdesign.md` were merged into this one file.)
...
| **M2** App shell, auth, routing | ✅ done | Sticky topbar (navy brand mark, blue active nav) + flat surfaces; migrated to the "Calm Efficiency" v1.0 system per `design.md` |
...
├── design.md                         # Canonical UI design system (Calm Efficiency v1.0)
├── src/
...
**The single source of truth is [`design.md`](design.md)** — the "Calm Efficiency"
v1.0 design system (navy/blue identity, violet-for-AI-only, semantic green/amber/red,
`--ds-*` tokens in `src/index.css`, the animated AI glow motif, and the honesty
rule against fabricated metrics). Follow it for all new work and all migrations;
don't duplicate its contents here.

Legacy note: older screens still use `--color-*` (flat-purple) tokens in
`index.css` — these are being migrated screen-by-screen (status tracked in
`design.md` §7). A separate `newdesign.md` was merged into `design.md`, which is
now the only design doc.
