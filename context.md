...
> Read this first when returning to the project. Last updated: 2026-06-07 (M5 evidence uploads + M6a OCR text extraction shipped via Power Automate flows; AIAgentFlow hook ready, no UI yet).
...
| **M6b** AI-assisted answer auto-population | not started | Wire the `AIAgentFlow` (hook exists as `useAskAI`, no UI). On evidence upload, feed extracted text + the assessment's questions to the LLM, write back suggested answers with confidence scores + an "AI populated" badge the assessor can verify or override. The biggest unshipped PRD feature. |
