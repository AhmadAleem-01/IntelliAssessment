...
> Read this first when returning to the project. Last updated: 2026-06-07 (M6b AI-assisted answer auto-population shipped: per-question evidence bindings authored in the template, assessment-time file-variable → uploaded-file mapping with persistence, batched LLM auto-fill with confidence-scored review + accept).
...
- **AI auto-fill (M6b)** — the headline AI feature, in two halves.
  - **Authoring (template editor).** Every Question's edit dialog has an **AI auto-fill** section with two inputs: an **Evidence file variable** (a stable placeholder like `q1-resume` the author types — they don't know the real upload names yet) and an **Extraction query** (natural-language instruction, e.g. *"If the extracted text contains a bachelor's degree, set this to true."*). Stored together as JSON in the existing `dnx_document_type_reference` column via `parseEvidenceBinding` / `serializeEvidenceBinding` (`features/templates/levels/evidenceBinding.ts`) — no schema change; legacy plain-text in that column is preserved as the `query`. See gotcha V.
  - **Assessment time.** An **AI auto-fill** button in the Evidence-files card header (hidden when PendingReview/Complete) opens `AiPopulateDialog`, a two-phase flow. **Phase 1 — Map:** every distinct file variable declared across the *unanswered, visible* questions is listed (with how many questions it drives) beside a dropdown of the real uploaded files. A name-match best guess pre-fills each row, and the assessor's effective mapping is **persisted on the instance** in `dnx_evidence_mapping` (`useSaveEvidenceMapping` + `parseEvidenceMapping` in `assessments/api.ts`) so reopening restores their picks — see gotcha W. **Phase 2 — Review:** `useAiPopulateMapped` extracts each *distinct* mapped file once (deduped — a file can back several variables), then fires **one batched `AIAgentFlow` call per file-variable group**, each prompt carrying only that group's questions + the author's per-question `instruction` (cost guardrail: one LLM call per file, not per question). Proposals render with a value, a one-line rationale, and a green/amber/red **confidence chip**; accept individually or **Accept all**. Nothing writes until accepted. Per-file failures surface as warnings instead of sinking the whole run.
...
- **Subsection-level file-variable catalog** — currently each question declares its own file variable freely. A subsection-level list of expected files (the catalog questions pick from) would tighten authoring and the mapping UI. Deferred earlier in favour of the per-question-only first slice.

### M8b — Custom letter builder + reason-grouped qualifications (deferred)

See the milestone-table row + [[project_letter_customisation]]. Author-controlled letter layout + reason-grouped qualifications. **Held until the user explicitly asks** — needs the reason model (free-text vs picklist) scoped first.

### M9 — Hardening (the remaining real milestone)

- **Role-gated actions** — Submit / Reopen / Delete / Approve / Reject / Mark resolved / AI auto-fill are all visible to every signed-in user. Wire Entra/Dataverse role detection (Assessor / Reviewer / Admin per PRD §5.1) and hide actions accordingly. Server-side enforcement (Dataverse security roles) is the real barrier; UI hiding is polish.
- **Persisted outcome pill on the global `/assessments` list** — the list shows status pills but not the computed/persisted outcome.
- **axe-core a11y scan** + keyboard-nav pass across every field type.
- **Perf check on a 200-question template** (PRD §8: ≤ 3 s) — virtualize long sections with `@tanstack/react-virtual` if needed.

### Smaller polish (any time)

- **Code-split with `React.lazy()`** — main bundle is ~1.68 MB / 474 kB gzipped (chunk-size warning on every build); route-level splitting would meaningfully cut first paint.
- **Throttle audit snapshots** — every keystroke (after debounce) writes one version row + one file. For long sessions this fills `dnx_assessment_versions` fast. Skip the snapshot when < ~30 s since the last Autosave snapshot; always snapshot on Submit/Reopen/Approve/Reject/Revert.
- **Multi-value visibility rules** — a rule's RHS is a single value; the contract is forward-compatible (`value: string[]` migrates with a parser update).
- **Mobile responsiveness audit** — topbar nav + tree editor untested at narrow viewports.
