...
> Read this first when returning to the project. Last updated: 2026-06-09 (M8b **fully shipped**: M8b.1 custom letter builder — drag-drop blocks on a new **Letter** tab, rich-text with inline colourable answer chips + a `/` slash menu, formatting toolbar — plus M8b.2 **Grouped subsections** block, which groups a chosen section's subsections by the answer to a question found inside each of them (matched by name, no fixed terminology — works for any "qualification"/"reason"-shaped structure). Also fixed: clearing a date answer sent `''` to `Edm.Date` → now sends null.)
