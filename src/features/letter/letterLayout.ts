...
/** How a background image fills the letter page. */
export type BackgroundMode = 'contain' | 'cover' | 'tile';

/** Nine anchor points for a scaled background image on the page. */
...
/**
 * Page-level letterhead settings — chrome that lives *outside* the block
 * stream. Unlike blocks, these appear once (header at the very top, footer at
 * the very bottom) and, in the Word export, become true repeating page
 * headers/footers. All fields optional; an unset header/footer falls back to
 * the built-in brand strip / "Generated by…" line so existing letters are
 * unchanged.
 *
 * The background image's *bytes* live in the template's `dnx_letter_background`
 * File column (see gotcha AB) — only its display settings live here. `image`
 * is a soft flag: `true` once an image has been uploaded, so the renderer knows
 * to download the image bytes off the template. Clearing the image sets it
 * back to `false`.
 */
export interface PageSettings {
  /** Rich HTML (same editor as heading/text). Empty = built-in brand strip. */
  header?: string;
  /** Rich HTML. Empty = built-in "Generated by IntelliAssessment · vN" footer. */
  footer?: string;
  /** Whether a background image has been uploaded to the template's Image column. */
  image?: boolean;
  /** How the background image fills the page. Default 'contain'. */
  backgroundMode?: BackgroundMode;
  /** Background image opacity 0–1 (so text stays readable over it). Default 0.15. */
  backgroundOpacity?: number;
  /**
   * Fraction of the page WIDTH the image occupies (0.1–1). Only meaningful for
   * contain/cover (tile ignores it). Default 1 = full page width. Aspect ratio
   * is always preserved, so this can never distort the image.
   */
  backgroundScale?: number;
  /** Where a scaled (<full-page) image anchors. Default 'center'. */
  backgroundPosition?: BackgroundPosition;
  /**
   * When true, a Fit-mode background bleeds to the paper edge instead of being
   * inset to the content margins. Only affects 'contain' (cover is always
   * full-bleed; tile always tiles the whole page). Default false = respect
   * margins.
   */
...
export interface LetterLayout {
  version: 1;
  blocks: LetterBlock[];
  /** Page-level letterhead (header / footer / background). Absent = defaults. */
  page?: PageSettings;
}
...
export function parseLetterLayout(stored: string | undefined | null): LetterLayout | undefined {
  ...
  try {
    ...
    return { version: 1, blocks, page: coercePage(parsed.page) };
  } catch {
    return undefined;
  }
}

/** Fill in page-level settings from raw JSON; returns undefined when absent. */
function coercePage(raw: unknown): PageSettings | undefined {
  if (!raw || typeof raw !== 'object') return undefined;
  const p = raw as Record<string, unknown>;
  const page: PageSettings = {};
  if (typeof p.header === 'string') page.header = p.header;
  if (typeof p.footer === 'string') page.footer = p.footer;
  if (p.image === true) page.image = true;
  if (p.backgroundMode === 'contain' || p.backgroundMode === 'cover' || p.backgroundMode === 'tile') {
    page.backgroundMode = p.backgroundMode;
  }
  if (typeof p.backgroundOpacity === 'number' && p.backgroundOpacity >= 0 && p.backgroundOpacity <= 1) {
    page.backgroundOpacity = p.backgroundOpacity;
  }
  if (typeof p.backgroundScale === 'number' && p.backgroundScale >= 0.1 && p.backgroundScale <= 1) {
    page.backgroundScale = p.backgroundScale;
  }
  if (
    typeof p.backgroundPosition === 'string' &&
    (BACKGROUND_POSITIONS as string[]).includes(p.backgroundPosition)
  ) {
    page.backgroundPosition = p.backgroundPosition as BackgroundPosition;
  }
  if (p.backgroundBleed === true) page.backgroundBleed = true;
  // Collapse an all-empty page object back to undefined so it round-trips clean.
...
export function serializeLetterLayout(layout: LetterLayout): string {
  return JSON.stringify({ version: 1, blocks: layout.blocks, page: layout.page });
}
